...
def agregar_concepto_endpoint(area_id):
    ...
    if not concepto_id:
        return {"error": "Falta 'concepto_id'"}, 400
    try:
        ok = agregar_concepto_a_area(area_id, concepto_id)
        if not ok:
            return {"message": "El concepto ya estaba asociado"}, 200
        return {"message": "Concepto agregado correctamente"}, 200
    except Exception as e:
        return {"error": str(e)}, 500
...
def agregar_fuente_endpoint(area_id):
    ...
    if not fuente_id:
        return {"error": "Falta 'fuente_id'"}, 400
    try:
        ok = agregar_fuente_a_area(area_id, fuente_id)
        if not ok:
            return {"message": "La fuente ya estaba asociada"}, 200
        return {"message": "Fuente agregada correctamente"}, 200
    except Exception as e:
        return {"error": str(e)}, 500
